...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        ...
        $quiz_id = create_quiz($pdo, $_SESSION['user_id'], $title, $description, 'school');
            ...
            foreach ($_POST['questions'] as $index => $question_data) {
                ...
                if ($question_type === 'mcq' && isset($question_data['answers'])) {
                    foreach ($question_data['answers'] as $answer_index => $answer_text) {
                        $is_correct = (isset($question_data['correct']) && $question_data['correct'] == $answer_index) ? 1 : 0;
                        add_answer($pdo, $question_id, sanitize_input($answer_text), $is_correct);
                    }
                }
            }
...
<head>
    ...
    <title>Créer un Quiz - <?php echo SITE_NAME; ?></title>
...
<body>
    ...
    <div class="container">
        <h1 style="margin: 2rem 0;">Créer un Quiz (École)</h1>
        ...
        <div class="card fade-in">
            <form method="POST" action="">
                ...
                <div style="display: flex; gap: 1rem; margin-top: 2rem;">
                    ...
                    <button type="submit" name="publish" class="btn btn-primary">Publier le quiz</button>
                </div>
            </form>
        </div>
    </div>
